...
var $fastFwd = $("#fastFwd");
//var $videoTimeComplete=$video[0].duration
...
function playVideo() {      
	if($video[0].paused) {
		...
		$playButton.find("img").attr("src", "http://173.255.192.230/site/libreriasJS/video_player/icons/pause-icon.png"); 
		$buttonControls.hide();
		$videoControls.css("margin-top", "5%");	     	
	} else {
		$video[0].pause();
		$playButton.find("img").attr("src", "http://173.255.192.230/site/libreriasJS/video_player/icons/play-icon.png");			
	}		
}
...
function muteVideo() {
	if ($video[0].muted === false) {
		$video[0].muted = true;
		$muteButton.find("img").attr("src", "http://173.255.192.230/site/libreriasJS/video_player/icons/volume-off-icon.png");
	} else {
		$video[0].muted = false;
		$muteButton.find("img").attr("src", "http://173.255.192.230/site/libreriasJS/video_player/icons/volume-on-icon.png");				
	}	
}
...
$video.on("timeupdate", function() {
	var $videoTime = $video[0].currentTime
        ...
        //console.log($videoTime);
        //alert($videoTime);
	if ($videoTime < $videoTimeComplete) {
		$duration.html(toHHMMSS($videoTime)+ "/"+toHHMMSS($videoTimeComplete));
	} else {
		$duration.html(toHHMMSS($videoTime)+ "/"+toHHMMSS($videoTimeComplete));
	}
});
...
function toHHMMSS(seconds) {
        ...
        if (seconds > 0) {return "00:"+seconds; }
        return "00:00";
    }   

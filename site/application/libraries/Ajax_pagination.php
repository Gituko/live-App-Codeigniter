...
class Ajax_pagination{
	...
	function create_links(){
		...
		if ($CI->uri->segment($this->uri_segment) != 0){
			...
			//if(strstr($_SERVER['REQUEST_URI'],'listedspacedetails1');
...
//echo $this->cur_page;exit;
		...
		//$this->base_url=site_base_url()."posts/ajaxPaginationData/";
  		// And here we go...
		$output = '';
		...
		?>
        <script>
        function <? echo $this->link_func ?>(page,start_date,end_date,parking_id){ 
		//alert(page);
            $.ajax({
                method: "POST",
                url: "<?php echo $this->base_url; ?>"+page,
                data: { page: page,start_date: start_date,end_date: end_date,parking_id: parking_id },
                beforeSend: function(){

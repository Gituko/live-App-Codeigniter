...
function get_quotations_manage_table($items,$controller)
{
	...
	$table='<table class="tablesorter" id="sortable_table">';
	$headers = array('<input type="checkbox" id="select_all" />', 
	$CI->lang->line('qoutations_qoutation_id'),
	$CI->lang->line('qoutations_qoutation_number'),
	$CI->lang->line('qoutations_qoutation_date'),
	$CI->lang->line('qoutations_customer_name'),
	$CI->lang->line('qoutations_grand_total'),
	$CI->lang->line('quotation_to_joborder'),
	//'&nbsp;'
	);
	$table.='<thead><tr>';
	$count = 0;
...
function get_quotations_data_row($item,$controller)
{
	...
	$table_data_row.='<td width="10%">'.$item->quotation_id.'</td>';
	$table_data_row.='<td width="15%">'.$item->quotation_no.'</td>';
	$table_data_row.='<td width="13%">'.date('d/m/Y',strtotime($item->quotation_date)).'</td>';
	$table_data_row.='<td width="11%">'.ucfirst($item->customer_fname).' '.ucfirst($item->customer_lname).'</td>';
    $table_data_row.='<td width="11%">'.to_currency($item->quotation_grand_total).'</td>';
	/*to_currency($item->cost_price)*/
	/*$table_data_row.='<td width="11%" align="right">'.to_currency($item->unit_price).'</td>';
	$table_data_row.='<td width="11%">'.to_quantity($item->quantity).'</td>';*/
	/*$table_data_row.='<td width="12%">'.anchor($controller_name."/inventory/$item->quotation_id/width~$width", lang('quotation_job_order'),array('class'=>'thickbox','title'=>lang($controller_name.'_count'))).'</td>';*///inventory details	
	
	/*$table_data_row.='<td width="12%">'.anchor($controller_name."/inventory/$item->quotation_id/width~$width", lang('quotation_job_order'),array('class'=>'thickbox','title'=>lang($controller_name.'_count'))).'</td>';*/
	$table_data_row.='<td width="14%" style="font-size:12px; padding:2px 7px;"><a onclick = "move_joborder('.$item->quotation_id.');">Convert to Job Order</td>';
	/*$table_data_row.='<td width="4%" class="rightmost">'.anchor($controller_name."/view/$item->item_id/width~$width", lang('common_edit'),array('class'=>'thickbox','title'=>lang($controller_name.'_update'))).'</td>';	*/	
...
function get_joborders_manage_table($items,$controller)
{
	...
	$table='<table class="tablesorter" id="sortable_table">';
	$headers = array('<input type="checkbox" id="select_all" />', 
	$CI->lang->line('joborder_joborder_id'),
	$CI->lang->line('joborder_joborder_number'),
	$CI->lang->line('joborder_joborder_date'),
	$CI->lang->line('joborder_customer_name'),
	$CI->lang->line('joborder_grand_total'),
	$CI->lang->line('joborder_details'),
	//'&nbsp;'
	);
	$table.='<thead><tr>';
	$count = 0;
...
function get_joborders_data_row($item,$controller)
{
	...
	$table_data_row='<tr>';
	$table_data_row.="<td width='3%'><input type='checkbox' id='item_$item->quotation_id' value='".$item->quotation_id."'/></td>";
	$table_data_row.='<td width="10%">'.$item->joborder_id.'</td>';
	$table_data_row.='<td width="15%">'.$item->joborder_no.'</td>';
	$table_data_row.='<td width="13%">'.date('d/m/Y',strtotime($item->joborder_date)).'</td>';
	$table_data_row.='<td width="11%">'.ucfirst($item->customer_fname).' '.ucfirst($item->customer_lname).'</td>';
    $table_data_row.='<td width="11%">'.to_currency($item->joborder_grand_total).'</td>';
	/*to_currency($item->cost_price)*/
	/*$table_data_row.='<td width="11%" align="right">'.to_currency($item->unit_price).'</td>';
	$table_data_row.='<td width="11%">'.to_quantity($item->quantity).'</td>';*/
	/*$table_data_row.='<td width="12%">'.anchor($controller_name."/inventory/$item->quotation_id/width~$width", lang('quotation_job_order'),array('class'=>'thickbox','title'=>lang($controller_name.'_count'))).'</td>';*///inventory details	
	...
	/*$table_data_row.='<td width="12%">'.anchor($controller_name."/inventory/$item->quotation_id/width~$width", lang('quotation_job_order'),array('class'=>'thickbox','title'=>lang($controller_name.'_count'))).'</td>';*/
	/*$table_data_row.='<td width="14%" style="font-size:12px; padding:2px 7px;"><a onclick = "move_joborder('.$item->quotation_id.');">Convart to Job Order</td>';*/
	$table_data_row.='<td width="5%" class="rightmost">'.anchor($controller_name."/view/1/width~$width", lang('common_edit'),array('class'=>'thickbox','title'=>lang($controller_name.'_update'))).'</td>';
	/*$table_data_row.='<td width="4%" class="rightmost">'.anchor($controller_name."/view/$item->item_id/width~$width", lang('common_edit'),array('class'=>'thickbox','title'=>lang($controller_name.'_update'))).'</td>';	*/	

...
{   
	...
	public  function home() 
	{
		...
		if($data["session_admin_user_name"]!='' && $data["session_admin_id"]!='')
		{
                    $this->templete->contenido = $this->load->view("dashboard/index",$data,true); //$this->load->view("paquete_credito/index", $data, TRUE);
	   ...
	   if ($this->input->server('REQUEST_METHOD') === 'POST')
			{
				...
				$advertisement_end_date1=$this->input->post('advertisement_end_date');
				$page=$this->input->post('page');
				
				$days=$this->input->post('advertisement_end_date');
				$total_price=$this->input->post('total_price');
				$advertisement_code=$this->input->post('advertisement_code');
	   ...
	   if ($this->input->server('REQUEST_METHOD') === 'POST')
			{
				...
				$emoji_type=$this->input->post('emoji_type');
				...
				$time=time();
		            if($_FILES['video']['name']!='')
					{
						if(!is_dir('../uploads/advertisement_video/'))
							...
							//$video=  str_replace(" ","_", $config['file_name']); //$config['file_name'];
							$this->load->library('upload', $config);
           						 $this->upload->initialize($config);
...
/****************************************************************************************************/
  ...
  public function delete_artist_follow($id='')
{
	$delete=$this->module->delete_artist_follow($id);

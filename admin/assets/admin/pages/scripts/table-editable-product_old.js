...
//alert("popo");
    var handleTable = function () {
        ...
        function editRow(oTable, nRow) {
			...
			var maincat_select_box='<div class="form-control height-auto"><div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 275px;"><div class="scroller" data-always-visible="1" style="overflow: hidden; width: auto; height: 275px;" data-initialized="1"><ul class="list-unstyled">';
			...
			jqTds[6].innerHTML = maincat_select_box;
			
			var select_box='<select class="form-control" name="status" id="status" ><option value="Active" ';
			if(aData[6]=='Active'){ select_box+='selected'; }else{select_box+='';}
			select_box+='>Active</option><option value="Inactive"';
			if(aData[6]=='Inactive'){ select_box+='selected'; }else{select_box+='';}
			select_box+='>Inactive</option></select>';
			
			
			
           jqTds[7].innerHTML = select_box;
        ...
        function saveRow(oTable, nRow,id,maincat) {
			...
			if(jqSelects[1].options[jqSelects[1].selectedIndex].value=='Active')
			{
				var css='success';
			}
			else if(jqSelects[1].options[jqSelects[1].selectedIndex].value=='Inctive')
			{
				var css='warning';
			}
			else
			...
			var selecte_val='<span class="label label-sm label-'+css+'">'+jqSelects[1].options[jqSelects[1].selectedIndex].value+'</span>';
			...
			oTable.fnUpdate(maincat, nRow, 6, false);
            oTable.fnUpdate(selecte_val, nRow, 7, false);
            oTable.fnUpdate('<a href="'+site_base_url+'edit_product/'+id+'" class="btn btn-xs default btn-editable"><i class="fa fa-pencil"></i> Edit</a>', nRow, 8, false);
